...
death_img = pygame.image.load(path.join(img_dir, 'end.png')).convert()
...
xingxing_img =pygame.image.load(path.join(img_dir, 'xingxing.jpg')).convert()
player_mini_img = pygame.transform.scale(xingxing_img, (20, 20))
...
def main_menu():
    ...
    # 加载游戏初始界面背景图片
    title = pygame.image.load(path.join(img_dir, "beijing.png")).convert()
    ...
    while True:
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            draw_text(screen, " Aircraft Battle", 40, WIDTH / 2, HEIGHT / 4)
            draw_text(screen, " ENTER to     start game", 30, WIDTH / 2, HEIGHT / 2)
            pygame.display.update()
...
while running:
    ...
    if player.lives == 0 and not death_explosion.alive():
        ...
        end = pygame.image.load(path.join(img_dir, "end.png")).convert()
        end = pygame.transform.scale(end, (WIDTH, HEIGHT), screen)
        screen.blit(end, (0, 0))
        pygame.display.update()
        pygame.time.wait(3000)
        menu_display = True
